...
@app.get("/api/capability-statement")
async def gen_capability(request: Request):
    u=gu(request);se=SessionLocal()
    try:
        co=se.query(Company).filter(Company.id==u["company_id"]).first()
        ...
        c.rect(0,h-104,w,4,fill=1)
        c.setFillColor(HexColor("#ffffff"))
        c.setFont("Helvetica-Bold",24)
        c.drawString(50,h-55,co.name or"Company Name")
        c.setFont("Helvetica",11)
        c.drawString(50,h-80,"Capability Statement")
        # Core info section
        y=h-140
        ...
        services=co.services or[]
        if services:
            c.drawString(50,y,"Services: "+", ".join(services[:8]))
            y-=16
        certs=co.certifications or[]
        if certs:
            c.drawString(50,y,"Certifications: "+", ".join(certs))
            y-=16
        regions=co.regions or[]
        if regions:
            c.drawString(50,y,"Regions: "+", ".join(regions))
            y-=16
        if co.bonding_capacity:
            c.drawString(50,y,"Bonding Capacity: $"+"{:,.0f}".format(co.bonding_capacity))
            y-=16
        # NAICS section
        y-=15
        c.setFillColor(HexColor("#1a1f2e"))
        c.setFont("Helvetica-Bold",13)
        c.drawString(50,y,"NAICS Codes")
        y-=5
        c.setStrokeColor(HexColor("#3b82f6"))
        c.line(50,y,200,y)
        y-=18
        ...
        c.setFont("Helvetica",10)
        naics=co.naics or[]
        for i in range(0,len(naics),4):
            row="    ".join(naics[i:i+4])
        ...
        c.setFont("Helvetica",10)
        if co.notify_email:
            c.drawString(50,y,"Email: "+co.notify_email)
            y-=16
        if co.notify_phone:
            c.drawString(50,y,"Phone: "+co.notify_phone)
            y-=16
        # Footer
        c.setFillColor(HexColor("#3b82f6"))
        ...
        c.drawCentredString(w/2,10,"Generated by ConstructBid AI — constructbidai.com")
        c.save()
        pdf_bytes=buf.getvalue()
        b64=base64.b64encode(pdf_bytes).decode()
        return{"pdf":b64,"filename":(co.name or"Company")+" Capability Statement.pdf"}
    finally:se.close()
